#include <stdio.h>

void DibujarRectangulo(ancho,alto,opcion)
{
	int i,j;
	
	for(i=1;i<=alto;i++)
	{
		for(j=1;j<=ancho;j++)
		{
			if (opcion==1)
			{
				if (j==1 || i==1 || i==alto || j==ancho)
				{
					printf("X");
				}
				else
				{
					printf(" ");
				}
			}
			else
			{
				if (opcion==2)
				{
					if (j==1 || i==1 || i==alto || j==ancho)
					{
						printf(" ");
					}
					else
					{
						printf("/");
					}
				}
				else
				{					
					if (opcion==3)
					{
						if (j==1 || i==1 || i==alto || j==ancho)
						{
							printf("X");
						}
						else
						{
							printf("/");
						}
					}
				}
			}
		
		}
		printf("\n");
	}
}

void pintarMenu()
{
	printf("____________________ \n");
	printf("1. Dibujar contorno \n");
	printf("2. Dibujar relleno \n");
	printf("3. Dibujar ambos \n");
	printf("____________________ \n");
}
int main(int argc, char *argv[]) {
	
	int alto, ancho, opcion;
	
	
	
	printf("Introduce el alto del retangulo \n");
	fflush(stdin);
	scanf("%d",&alto);
	printf("Introduce el ancho del retangulo \n");
	fflush(stdin);
	scanf("%d",&ancho);
	printf("\n");
	
	pintarMenu();
	
	
	printf("Elige una opcion \n");
	
	do
	{
		fflush(stdin);
		scanf("%d",&opcion);
		 switch (opcion){
			case 1: DibujarRectangulo(ancho,alto,1);
				 break;
			case 2: DibujarRectangulo(ancho,alto,2);
				 break;
			case 3: DibujarRectangulo(ancho,alto,3);
				break;
			default:
				printf("Elige una opcion valida\n");
		 }
	} while (opcion!= 1 && opcion!= 2 && opcion!= 3) ;
	
	return 0;
}

